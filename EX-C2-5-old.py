...
expression = f"Advisory(((#{HR_PARAM_ID} > {HR_UPPER_THRESHOLD}) OR (#{HR_PARAM_ID} < {HR_LOWER_THRESHOLD})) AND ((#{ABP_MEAN_PARAM_ID} > {ABP_UPPER_THRESHOLD}) OR (#{ABP_MEAN_PARAM_ID} < {ABP_LOWER_THRESHOLD})),'EX-C2-5-postoperativebleeding',  2,'Potential post-operative bleeding')"
